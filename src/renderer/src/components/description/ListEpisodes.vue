<template>
    <div v-if="episodes" class="mt-10 grid gap-2" :class="{
        ' grid-cols-4': toogleBar,
        ' grid-cols-5': toogleMenu,
        ...
        <RouterLink v-if="toogleList" :to="{ path: `/list/${anime.slug}/${episode.anime_episode_number}` }"
            ...
            <p>
                ...
                {{ new Date(episode.created_at).toISOString().slice(0, 10) }}
            </p>
        </RouterLink>
        <div v-if="
        ...
        " class="flex w-48 bg-gray-600 dark:bg-neutral-900 rounded-lg">
            ...
            <div class="flex flex-row gap-1 p-2">
                <p class="font-medium">
                    <span v-if="nextEpisode(episodes[episodes.length - 1].created_at) > 0">
                        za {{ nextEpisode(episodes[episodes.length - 1].created_at) }} dni</span>
