      ...
      <button @click="
        ...
        <div class="bg-gray-900 dark:bg-neutral-900 bg-opacity-60 rounded-md overflow-hidden flex w-80 h-24">
          ...
          <div class="flex flex-col items-start justify-start w-full">
            ...
            <p class="text-left text-sm font-extralight p-1">
              {{ item[0].title.length <= 70 ? item[0].title : `${item[0].title.slice(0, 19)}...` }} </p>
          </div>
        </div>
      </button>
    </div>
        ...
        if (Array.isArray(value.entry)) {
          value.entry.forEach((link) => {
            window.electron.ipcRenderer.send('getApiFour', [
              `https://api.docchi.pl/v1/series/related/${link.mal_id}`,
              value.relation
            ])
          })
        }
