<template>
    <Carousel class="w-11/12" ref="myCarousel" :itemsToShow="itemsShow" snapAlign="start" :autoplay="autoplay"
        ...
        <Slide v-for="(item, index) in data" :key="index">
            <RouterLink :to="item.episodes > 1 || item.episodes == null
                ? { path: `/list/${item.slug}` }
                : { path: `/list/${item.slug}/1` }
                " class="group h-[370px] w-[250px] relative rounded-lg bg-red-400" :key="item.mal_id" :class="{
                    'h-[200px] w-[150px]': itemsShow >= 9,
                    'h-[310px] w-[200px]': itemsShow > 7 && itemsShow < 9
                }">
                ...
                <div class="absolute bottom-0 w-full p-2">
                    <h1 class="text-gray-100 group-hover:text-white">
                        {{ item.title.length <= 20 ? item.title : `${item.title.slice(0, 19)}...` }} </h1>
                </div>
            </RouterLink>
        </Slide>
        <template #addons>
